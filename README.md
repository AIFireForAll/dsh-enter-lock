...
- `Ctrl+Alt+L` 等 `Ctrl+Alt` 组合在 Windows 上不可靠：系统把 `Ctrl+Alt` 映射为 AltGr，且微软拼音、微信输入法等输入法（包括其英文模式）会在按键到达网页前把它消费掉；
- `Ctrl+Space` 是系统输入法切换键，不可用；
- `Ctrl+L` 是 Chrome / Edge 的地址栏快捷键，`Ctrl+Shift+L` 是 Edge 的「粘贴并搜索」，均被浏览器保留；
- `Ctrl+Shift+K` 在上述输入法环境下可正常到达页面并切换锁定状态，因此作为正式组合键。
