window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Keyboard shortcut: Ctrl+Shift+K on Windows/Linux, Cmd+Shift+K on macOS.
     * The shortcut only fires while focus is inside a composer card, because
     * the handler resolves the session id from the focused element's card
     * (see sessionIdFromTarget). IME composition and key auto-repeat are
    ...
    /**
     * The lock shortcut: Ctrl+Shift+K on Windows/Linux, Cmd+Shift+K on macOS.
     *
     * Combo selection, verified on device:
     * - Ctrl+Alt combos (e.g. Ctrl+Alt+L) are unreliable on Windows: the OS
     *   maps Ctrl+Alt to AltGr and active IMEs (Microsoft Pinyin, WeChat IME,
     *   ...) consume the keydown before the page sees it — even in their
     *   English mode. Measured with a capture-phase listener that never
     *   received the event.
     * - Ctrl+Space is the system IME toggle — never usable.
     * - Ctrl+L is the address bar in Chrome and Edge; Ctrl+Shift+L is Edge's
     *   paste-and-search — both reserved.
     * - Ctrl+Shift+K reaches the page and toggles on the same machine with
     *   both IMEs, so it is the shipped combo.
     *
     * Matching prefers the physical key (event.code): an active Windows IME
     * may report key="Process" (keyCode 229) for the combo without composing,
     * but event.code stays "KeyK".
     */
    function isLockShortcut(event) {
      if (event.repeat) return false
      var key = event.key || ''
      var isK = event.code === 'KeyK' || key.toLowerCase() === 'k'
      if (!isK) return false
    ...
    /**
     * True when the composer card containing target hosts an open trigger
     * candidate menu with a highlighted candidate. That menu (the @ file /
     * session picker, the / command menu, ...) consumes Enter itself: the
     * official composer keyboard arbitration picks the candidate instead of
     * submitting. It renders as a [role="listbox"] whose aria-activedescendant
     * attribute exists exactly while a candidate is highlighted — the same
     * condition under which the official composer returns "pick-highlighted"
     * instead of routing Enter to submit.
     */
    function triggerMenuConsumesEnter(target) {
      if (!(target instanceof Element)) return false
      var card = target.closest('[data-composer-card]')
      if (card === null) return false
      return card.querySelector('[role="listbox"][aria-activedescendant]') !== null
    }
    ...
    function apply(ctx) {
      ...
      ctx.effect(function () {
        function onKeyDown(event) {
          // Lock toggle shortcut; only fires with focus inside a composer card.
          if (SHORTCUT_ENABLED && isLockShortcut(event)) {
            // Only a real composition session skips the shortcut. Deliberately
            // NOT gating on keyCode 229: an active Windows IME reports the
            // combo as key="Process" / keyCode 229 without composing; the
            // matcher works on event.code, which stays "KeyK" regardless.
          ...
          // Only block Enter that would actually reach the official submit
          // path. When a trigger candidate menu is open with a highlighted
          // candidate, the official composer consumes Enter to confirm that
          // candidate and never sends — leave those keys alone.
          if (triggerMenuConsumesEnter(event.target)) return
          event.preventDefault()
